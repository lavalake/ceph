...
void PG::scrub(ThreadPool::TPHandle &handle)
{
  ...
  if (!scrubber.active) {
    assert(backfill_targets.empty());
    // XXX: Use get_min_acting_features()?
    assert(get_min_acting_features() & CEPH_FEATURE_CHUNKY_SCRUB);
...
PG::RecoveryState::Backfilling::react(const RemoteReservationRejected &)
{
  ...
  for (set<pg_shard_t>::iterator it = pg->backfill_targets.begin();
    ...
    if (con) {
      assert(pg->get_osdmap()->get_xinfo(it->osd).features & CEPH_FEATURE_BACKFILL_RESERVATION);
...
PG::RecoveryState::WaitRemoteBackfillReserved::react(const RemoteBackfillReserved &evt)
{
  ...
  if (backfill_osd_it != context< Active >().remote_shards_to_reserve_backfill.end()) {
    ...
    if (con) {
      assert(pg->get_osdmap()->get_xinfo(backfill_osd_it->osd).features & CEPH_FEATURE_BACKFILL_RESERVATION);
      pg->osd->send_message_osd_cluster(
        new MBackfillReserve(
